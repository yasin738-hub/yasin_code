...
if(isset($_POST['add'])){
$additem =$database->prepare("INSERT INTO todolist(text,userid,status) VALUES(:text,:userid,'status')");
...
echo'<td><form><button class="btn btn-outline-success" 
type="submit" name="status" value="'.$item['id'].'"> منجز</button></td>';
}

echo'<td><button class="btn btn-outline-danger" type="submit" name="remov" value"'.$item['id'].'">حذف</button></form></td>';
echo'</tr>';
...
$updateStatus = $database->prepare("UPDATE todolist SET status ='on' WHERE id =:id");
$updateStatus->bindParam("id",$_GET['status']);
$updateStatus->execute();
header("location:todolist.php");
    }
